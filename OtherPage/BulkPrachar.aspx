...
<head runat="server">
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .result-modal-card{text-align:center;padding:28px 22px 22px}
    .result-icon{width:64px;height:64px;margin:0 auto 16px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:30px;font-weight:700}
    .result-icon.success{color:#fff;background:var(--green);box-shadow:0 10px 24px rgba(30,122,58,.28)}
    .result-icon.error{color:#fff;background:#c0392b;box-shadow:0 10px 24px rgba(192,57,43,.22)}
    .result-title{font-family:'Noto Sans Gujarati','Source Sans 3',sans-serif;font-size:20px;font-weight:700;color:var(--text);line-height:1.4;margin-bottom:10px}
    .result-message{font-family:'Noto Sans Gujarati','Source Sans 3',sans-serif;font-size:14px;line-height:1.7;color:var(--muted);margin-bottom:8px}
    .result-sub{font-family:'Noto Sans Gujarati','Source Sans 3',sans-serif;font-size:12px;font-weight:600;color:var(--gold-dark);margin-bottom:18px}
    .result-actions{display:flex;justify-content:center}
    .result-actions .btn{min-width:140px;padding:13px 18px}
  </style>
</head>
<body>
  <form id="form1" runat="server">
    <asp:HiddenField ID="hfPracharType" runat="server" ClientIDMode="Static" />
    <asp:HiddenField ID="hfEnquiryResult" runat="server" ClientIDMode="Static" />
    <div class="wrap">
      <div class="brand">
    ...
    <div class="modal" id="detailModal" aria-hidden="true">
      <div class="modal-card" role="dialog" aria-modal="true" aria-label="Bulk Prachar Detail">
        ...
        <div class="modal-footer">
            <asp:Button Text="Send Enquiry" OnClick="btn_enquiry_Click" CssClass="btn btn-wa" ID="btn_enquiry" runat="server"
                CausesValidation="false" UseSubmitBehavior="true"
                OnClientClick="return confirm('Are you sure you want to send this enquiry?');" />
        </div>
      </div>
    </div>

    <div class="modal" id="enquiryResultModal" aria-hidden="true">
      <div class="modal-card result-modal-card" role="dialog" aria-modal="true" aria-labelledby="enquiryResultTitle">
        <div class="result-icon success" id="enquiryResultIcon">&#10003;</div>
        <div class="result-title" id="enquiryResultTitle">તમારી ઇન્ક્વાયરી મોકલાઈ ગઈ</div>
        <div class="result-message" id="enquiryResultMessage">
          તમારી ઇન્ક્વાયરી સફળતાપૂર્વક મોકલાઈ ગઈ છે. અમારી ટીમ તમારી વિનંતીની સમીક્ષા કરશે અને ટૂંક સમયમાં તમારો સંપર્ક કરશે.
        </div>
        <div class="result-sub" id="enquiryResultSub">BJP WINGS પસંદ કરવા બદલ આભાર.</div>
        <div class="result-actions">
          <button type="button" class="btn btn-wa" data-close-result>ઠીક છે</button>
        </div>
      </div>
    </div>
  </form>
  ...
  <script type="text/javascript">
    window.pracharRateData = <%= RateDataJson ?? "{}" %>;
    ...
    (function () {
      ...
      function buildRateHtml(data) {
        ...
        var rows = [
          ...
          ['Total Voter', data.totalVoters],
          ['Total Mobile No', data.totalMobileNos],
          [data.unitCostLabel, data.unitCostValue],
      ...
      function openDetail(titleText, videoId, rateType) {
        ...
        var hfPracharType = document.getElementById('hfPracharType');
        if (hfPracharType) hfPracharType.value = rateType || 'sleep';
      ...
      window.addEventListener('keydown', function (e) {
        if (e.key === 'Escape') {
          var resultModal = document.getElementById('enquiryResultModal');
      ...
      function closeEnquiryResult() {
        var resultModal = document.getElementById('enquiryResultModal');
        if (!resultModal) return;
        hideModal(resultModal);
      }

      window.showEnquiryResult = function (mode, message) {
        var resultModal = document.getElementById('enquiryResultModal');
        var resultIcon = document.getElementById('enquiryResultIcon');
        var resultTitle = document.getElementById('enquiryResultTitle');
        var resultMessage = document.getElementById('enquiryResultMessage');
        var resultSub = document.getElementById('enquiryResultSub');
        if (!resultModal) return;
        ...
        if (resultTitle) {
          resultTitle.textContent = isSuccess ? 'તમારી ઇન્ક્વાયરી મોકલાઈ ગઈ' : 'ઇન્ક્વાયરી મોકલી શકાઈ નથી';
        }
        if (resultMessage) {
          resultMessage.textContent = message || (isSuccess
            ? 'તમારી ઇન્ક્વાયરી સફળતાપૂર્વક મોકલાઈ ગઈ છે. અમારી ટીમ તમારી વિનંતીની સમીક્ષા કરશે અને ટૂંક સમયમાં તમારો સંપર્ક કરશે.'
            : 'ઇન્ક્વાયરી મોકલતી વખતે કંઈક ખોટું થયું. કૃપા કરીને ફરી પ્રયાસ કરો.');
        }
        if (resultSub) {
          resultSub.textContent = isSuccess ? 'BJP WINGS પસંદ કરવા બદલ આભાર.' : 'કૃપા કરીને થોડી વાર પછી ફરી પ્રયાસ કરો.';
          resultSub.style.display = '';
        }
      ...
      document.addEventListener('click', function (e) {
        ...
        var resultModal = document.getElementById('enquiryResultModal');
        if (resultModal && e.target === resultModal) closeEnquiryResult();
      });

      var hfEnquiryResult = document.getElementById('hfEnquiryResult');
      if (hfEnquiryResult && hfEnquiryResult.value) {
        try {
          var enquiryPayload = JSON.parse(hfEnquiryResult.value);
          if (enquiryPayload && enquiryPayload.mode) {
            window.showEnquiryResult(enquiryPayload.mode, enquiryPayload.message || '');
          }
        } catch (err) { }
        hfEnquiryResult.value = '';
      }
    })();
  </script>
</body>
